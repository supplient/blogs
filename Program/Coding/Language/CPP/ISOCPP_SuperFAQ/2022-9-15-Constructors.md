...
对象的初始化顺序：先基类（从左到右），再成员对象（从上到下），最后对象本身。
...
这一“自动回收”机制是通过调用`new`对应的`delete`来实现的（不含调用构造函数、析构函数的部分），
所以如果自定义了一个`operator new`的话，那么也应自定义一个`operator delete`，从而触发自动回收机制，避免内存泄漏。
